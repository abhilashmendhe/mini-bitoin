...
impl Tx {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, BTCErr> {
        ...
        if self.segwit {
            // println!("len of witness: {}", &self.witness.len());
            let mut i = 0;
            while i < self.witness.len() {
                result.push(2);
                let sig = &self.witness[i];
                result.extend(script_encode_variant(sig.len() as u64));
                result.extend(sig);
                i += 1;

                let pubkey = &self.witness[i];
                result.extend(script_encode_variant(pubkey.len() as u64));
